...
searchField.addEventListener('keypress',async(event) => {
  ...
  if(event.key === 'Enter'){
    ...
    if(foundProducts.length === 0){
      notFound.innerHTML = `
        <h2 class="text-2xl text-orange-500 text-center">Not Found</h3>
      `
      return;
    }
    ...
    foundProducts.forEach(product =>{
      ...
      <figure><img src="${image}" class="h-60 w-full" alt="car!"></figure>
      <div class="card-body">
        <h2 class="card-title">${category}</h2>
        <p>${title.length > 20 ?title.slice(0,20):title}</p>
        <div onclick="showModal('${description}','${image}') " class="card-actions justify-end">
